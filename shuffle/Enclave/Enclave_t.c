...
static sgx_status_t SGX_CDECL sgx_printf_helloworld(void* pms)
{
	...
	printf_helloworld(_in_p_return_ptr, _tmp_len, ms->ms_num);
	if (_in_p_return_ptr) {
...
static sgx_status_t SGX_CDECL sgx_compute_histogram(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	int* _tmp_p_return_ptr = ms->ms_p_return_ptr;
	size_t _tmp_len = ms->ms_len;
	size_t _len_p_return_ptr = _tmp_len;
	int* _in_p_return_ptr = NULL;
	...
	if (_tmp_p_return_ptr != NULL && _len_p_return_ptr != 0) {
		...
		if ((_in_p_return_ptr = (int*)malloc(_len_p_return_ptr)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	compute_histogram(_in_p_return_ptr, _tmp_len, ms->ms_num);
	if (_in_p_return_ptr) {
		if (memcpy_s(_tmp_p_return_ptr, _len_p_return_ptr, _in_p_return_ptr, _len_p_return_ptr)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
